...
Commands that expect an `array` reject non-array receivers (`player forEach …` → `STS2003`). Mutating a homogeneous `T[]` (`pushBack`, `set`, `append`, …) checks the value against `T` (`STS2004`). Fixed-shape tuples check `set` against the slot type at a constant index and reject length-changing mutations (`pushBack`, `append`, …). Untyped `array` stays gradual. Follow-ons such as `readonly T[]` and safer `#` indexing are listed under [Missing Features](Missing-Features) (bundle **B-ArraySoundness**).
